...
public class MigrationScriptReaderImpl implements MigrationScriptReader {
  ...
  /**
   * @param locations                       Locations of migrations scripts, e.g
   *                                        classpath:es/migration or file:/home/migration
   ...
   * @param lineSeparator                   Line separator. should be '\n' per default and only
   *                                        something else for backward compatibility / checksum
   *                                        stability
   * @param trimTrailingNewlineInMigrations Whether to remove a trailing newline in migration
   *                                        scripts.
   */
  ...
  private Stream<RawMigrationScript> readScriptsFromClassPath(String location) {
    ...
    try (ScanResult scanResult = new ClassGraph()
      ...
      scanResult.getAllResources()
          .filter(
              resource -> isValidFilename(Paths.get(resource.getPath()).getFileName().toString()))
          .forEach(resource -> {
            logger.debug("reading migration script '{}' from classpath...", resource);
            try (BufferedReader bufferedReader = new BufferedReader(
                new InputStreamReader(new ByteArrayInputStream(resource.load()), encoding))) {
