...
public class AggregateByPropertyValueCollectorDataFilterTypeTest extends
    AbstractAggregateByPropertyValueCollector {
  ...
  @Test
  void shouldFilterProductsByCategory() throws IOException {
    ...
    dataSource.send(data3.addMetadata(Properties.empty().withType(TEST_TYPE)));
    await().until(() -> webResourceSink.received().size() == 1);
    ...
    JsonPath.parse(jsonNode1).read("$.values[*].id", ArrayNode.class)
        .forEach(jsonNode -> assertTrue(Set.of(PRODUCT_3_ID)
