...
/**
 * This is the centre point of Elasticsearch-Evolution, and for most users, the only class they will
 * ever have to deal with.
 * <p>
 * It is THE public API from which all important Elasticsearch-Evolution functions such as migrate
 * can be called.
 * </p>
...
public class ElasticsearchEvolution {
  ...
  /**
   * This is your starting point. This creates a configuration which can be customized to your needs
   * before being loaded into a new ElasticsearchEvolution instance using the load() method.
   * <p>In its simplest form, this is how you configure Flyway with all defaults to get
   * started:</p>
   * <pre>
   * ElasticsearchEvolution esEvolution = ElasticsearchEvolution.configure()
   *     .load(RestClient.builder(HttpHost.create(esUrl)).build());
   *  </pre>
   * <p>After that you have a fully-configured ElasticsearchEvolution instance at your disposal
  ...
  public ElasticsearchEvolution(ElasticsearchEvolutionConfig elasticsearchEvolutionConfig,
    ...
    this.config = requireNonNull(elasticsearchEvolutionConfig,
        "elasticsearchEvolutionConfig must not be null")
        .validate();
    ...
    if (getConfig().isEnabled()) {
      ...
      if (rawMigrationScripts.size() > getConfig().getHistoryMaxQuerySize()) {
        throw new MigrationException(
            "configured historyMaxQuerySize of '%s' is to low for the number of migration scripts of '%s'".formatted(
