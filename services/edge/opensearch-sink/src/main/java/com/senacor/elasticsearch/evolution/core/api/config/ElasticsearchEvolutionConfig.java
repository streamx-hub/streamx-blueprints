...
public class ElasticsearchEvolutionConfig {
  ...
  /**
   * Locations of migrations scripts. Supported is classpath:some/path and file:/some/path The
   * location is scanned recursive. NOTE: all scripts in all locations / subdirectories will be
   * flatted and only the version number will be used to order them.
   */
  ...
  /**
   * Line separator, used only temporary between reading raw migration file line-by-line and parsing
   * it later.
   * <p>
   * NOTE: Only needed for backward compatibility / checksum stability!
   * <p>
   * Should be one of - '\n' (LF - Linux/Unix/OS X) - '\r' (CR - Classic MAC OS) - '\r\n' (CRLF -
   * Windows)
   */
  private String lineSeparator = "\n";

  /**
   * This content type will be used as default if no contentType header is specified in the header
   * section of a migration script. If no charset is defined, the {@link #encoding} charset is
   * used.
   */
  ...
  /**
   * The maximum query size while validating already executed scripts. This query size have to be
   * higher than the total count of your migration scripts.
   */
  private int historyMaxQuerySize = 1_000;

  /**
   * Whether to fail when a previously applied migration script has been modified after it was
   * applied.
   */
  ...
  /**
   * version to use as a baseline. The baseline version will be the first one applied, the versions
   ...
   * <p>
   * If you already have versions 1.0 and 3.0 applied, and now a version 2.0 is found, it will be
   * applied too instead of being rejected.
   */
   ...
   */
  public ElasticsearchEvolutionConfig validate()
    ...
    if (enabled) {
      ...
      if (placeholderReplacement) {
        ...
        placeholders.forEach((name, value) -> {
          requireCondition(name, s -> !s.contains(placeholderPrefix),
              "placeholder name '%s' must not contain placeholderPrefix '%s'", name,
              placeholderPrefix);
          requireCondition(name, s -> !s.contains(placeholderSuffix),
              "placeholder name '%s' must not contain placeholderSuffix '%s'", name,
              placeholderSuffix);
          requireCondition(value, s -> !s.contains(placeholderPrefix),
              "placeholder value '%s' must not contain placeholderPrefix '%s'", value,
              placeholderPrefix);
          requireCondition(value, s -> !s.contains(placeholderSuffix),
              "placeholder value '%s' must not contain placeholderSuffix '%s'", value,
              placeholderSuffix);
        });
      }
      requireNotBlank(historyIndex, "historyIndex must not be empty");
      requireCondition(historyMaxQuerySize, size -> size > 0,
          "historyMaxQuerySize value '%s' must be greater than 0", historyMaxQuerySize);
      ...
      requireCondition(baseline, version -> version.isAtLeast("1"),
          "baselineVersion '%s' must be at least 1", baseline);
    }
    return this;
  }
  ...
  public ElasticsearchEvolutionConfig setTrimTrailingNewlineInMigrations(
