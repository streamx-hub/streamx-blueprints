...
class AdjustImgSrcFunctionTest {
  ...
  private InMemorySource<CloudEvent> resourcesChannel;
  private InMemorySink<CloudEvent> outgoingResourcesSink;
  ...
  @BeforeEach
  void init() {
    resourcesChannel = connector.source(Channels.INCOMING_RESOURCES);
    outgoingResourcesSink = connector.sink(Channels.OUTGOING_RESOURCES);
    outgoingResourcesSink.clear();
  ...
  void shouldAdjustImgSrcOnEveryPublicationOfTheSamePage() throws IOException {
    ...
    resourcesChannel.send(editedPageEvent);
  ...
  void shouldNotAdjustAlreadyAdjustedPage() throws IOException {
    ...
    resourcesChannel.send(adjustedPageEvent);
  ...
  private void publishOptimizedImage(File imageFile) throws IOException {
    ...
    // when
    resourcesChannel.send(imageEvent);
  ...
  private List<CloudEvent> getOutgoingPages() {
    return outgoingResourcesSink.received().stream()
        .map(Message::getPayload)
