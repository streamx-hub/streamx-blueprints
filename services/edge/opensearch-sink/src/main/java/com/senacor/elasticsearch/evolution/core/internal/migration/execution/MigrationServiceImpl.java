...
public class MigrationServiceImpl implements MigrationService {
    ...
    try {
      ...
      if (!historyRepository.unlock()) {
        throw new MigrationException(
            "could not release the elasticsearch-evolution history index lock! Maybe you have to release it manually.");
      }
    }
  ...
  ExecutionResult executeScript(ParsedMigrationScript scriptToExecute) {
    ...
    try {
      Request request = new Request(
          scriptToExecute.getMigrationScriptRequest().getHttpMethod().name(),
      ...
      if (null != scriptToExecute.getMigrationScriptRequest().getBody()
        ...
        if (null == contentType.getCharset()) {
          logger.debug("no charset is defined for {}, setting to configured encoding {}",
              scriptToExecute.getFileNameInfo(), encoding);
          contentType = contentType.withCharset(encoding);
        }
        request.setEntity(
            new NStringEntity(scriptToExecute.getMigrationScriptRequest().getBody(), contentType));
      }
      ...
      error = Optional.of(new MigrationException(
          "execution of script '%s' failed".formatted(scriptToExecute.getFileNameInfo()), e));
    }
  ...
  List<ParsedMigrationScript> getPendingScriptsToBeExecuted(
    ...
    if (outOfOrder) {
      for (MigrationScriptProtocol protocol : history) {
        final ParsedMigrationScript parsedMigrationScript = scriptsInFilesystemMap.get(
            protocol.getVersion());
  ...
  private void validateOnMigrateIfEnabled(MigrationScriptProtocol protocol,
    ...
    if (validateOnMigrate && protocol.isSuccess()
        && protocol.getChecksum() != parsedMigrationScript.getChecksum()) {
      throw new MigrationException((
          """
  ...
  void waitUntilUnlocked() {
    while (historyRepository.isLocked()) {
      try {
        int waitTime = RandomUtils.getRandomInt(waitUntilUnlockedMinTimeInMillis,
            waitUntilUnlockedMaxTimeInMillis);
        logger.info("Elasticsearch-Evolution history index is locked, waiting {}ms until retry...",
            waitTime);
